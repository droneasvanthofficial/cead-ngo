...
Known items carried over from earlier work that have **not** been verified against CEAD source material, and should be confirmed with the client before being treated as fact: the `trustees` array in `BoardTrustees.jsx` and the `staffMembers` array in `StaffDetails.jsx`.

## Imported Claude Cowork project instructions

I need to redesign a already existing website in to a premium websites
